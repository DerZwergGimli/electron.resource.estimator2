  ...
  for (let i = 0; i < useAppStorage().assignmentsList.length; i++) {
    ...
    let body_assignments = [
      [
        ...
        {
          content:
            current_host?.cpu.sockets.toString().padEnd(5, ' ') ?? 'error',
          colSpan: 1,
        },
        {
        ...
        {
          content:
            current_host?.storage.amount.toString().padEnd(5, ' ') ?? 'error',
          colSpan: 1,
        },
        {
          content:
            current_host?.storage.size.toString().padEnd(5, ' ') ?? 'error',
          colSpan: 1,
        },
        {
          content:
            current_host?.storage.raid.toString().padEnd(5, ' ') ?? 'error',
          colSpan: 1,
        },
      ],
    ]
