...
export function draw_CodePage(doc: jsPDF) {
    ...
    const split_text = doc.splitTextToSize(JSON.stringify(useAppStorage().export(), null, 3), doc.internal.pageSize.getWidth() - 20)
    ...
    split_text.forEach((element, index) => {
        ...
        if (index_fix === 0) {
            doc.text(
                element,
                10, 25 + current_height
            )
        } else {
            doc.text(
                element,
                10, 10 + current_height
            )
        }
